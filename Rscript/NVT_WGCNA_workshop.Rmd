...
To run the WGCNA pipeline, we need log2fc values as input. We have already performed a DESeq2 analysis, so today we can fully focus on the WGCNA tutorial. For this workshop, we selected one compound (Nitrofurantoin), across three timepoints (2hr, 8hr, and 24hr), and three concentrations (5uM, 25uM, and 125uM).
...
Calculating the TOM can be time-consuming (on my computer, it took 24 minutes using 10 cores). Therefore, the results of this step have been precomputed and saved in the output folder. You can load them in the following code chunk. The code to calculate the TOM yourself has been commented out, so you can still run it if you want to.
...
_cutHeight_: In hierarchical clustering, this parameter defines the height at which to cut the dendrogram to form distinct modules. It determines the granularity of module detection: lower values result in fewer, larger modules, while higher values yield more, smaller modules. Choosing an optimal `cutHeight` involves balancing between capturing distinct gene groups and avoiding excessive fragmentation of modules.
